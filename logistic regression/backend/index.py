...
app = FastAPI(title="EcoPredict Brain API")
...
def get_resources():
    """Robust resource loading using absolute paths."""
...
@app.get("/")
def health_check():
    return {"status": "online", "message": "EcoPredict Brain is READY", "python": sys.version}
...
def predict(request: PredictionRequest):
    try:
        ...
        if c_code is None or r_code is None:
            raise HTTPException(status_code=400, detail=f"Invalid country: {request.country}")
